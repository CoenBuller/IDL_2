...
IMG_SIZE = 64
N_POINTS = 100000
TOTAL_IMAGES = 18000   # identico a hopalong, modifica se serve
...
# =====================
def generate_fractal_by_type(width=IMG_SIZE, height=IMG_SIZE, max_attempts=10, current_cmap=None):
    attempts = 0
    ...
    while attempts < max_attempts:
        ...
        try:
            df = trajectory(fractal_dream, *params, n=N_POINTS)
...
def generate_fractal_fallback(used_signatures, current_cmap):
    cmap_list = [inferno, viridis, fire]
    fallback_params = [
        [0.1, 0.1, 1.8, -1.2, 0.5, -0.8],
        [0.3, -0.4, 1.5, -1.5, 0.8, -0.2],
    ]
    ...
    for params in fallback_params:
        ...
        if sig not in used_signatures:
            ...
            return pil_img, (params, current_cmap)
...
def create_balanced_fractal_dataset(total_images=TOTAL_IMAGES):
    ...
    print(f"\nGenerazione dataset FRACTAL DREAM: {per_palette} immagini per palette")
    ...
    for current_cmap in [0, 1, 2]:
        ...
        while count < per_palette:
            img, out = generate_fractal_by_type(current_cmap=current_cmap)

            if img is None:
                img, out = generate_fractal_fallback(used_signatures, current_cmap)
    ...
    print(f"\nTempo totale: {elapsed//60:.0f} min {elapsed%60:.1f} s")

    dataset = np.array(dataset)
    labels = np.array(labels)

    np.save("dream_images.npy", dataset)
    np.save("dream_labels.npy", labels)
